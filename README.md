...
<p> 4. The three most important features were "gas","diesel", and "automatic" in that order, according to the permutation_importance function using the the hold-out cross-validation and Ridge regression model.</p>
...
<p> 6. It is thought that the number of feature could be reduced by applying PCA, and some clustering algorithm, making sure that removing a given feature, the clustering results are not affected.</p>
